...
class Group extends Controller {
														...
														form_dropdown('prinumber',$this->systemmodel->getPriList(isset($itemDetail['prinumber'])?$itemDetail['prinumber']:'','1'),(isset($itemDetail['prinumber']) && $itemDetail['prinumber']>20000)?$itemDetail['prinumber']:(($id!="")?'0':''),"id='prinumber' class='form-control required' data-style='input-sm btn-default'")
															:(($field['fieldname']=='bday')?
											                 ...
											                 .((isset($itemDetail[$field['fieldname']]) && $itemDetail[$field['fieldname']]!='' && file_exists('sounds/'.$itemDetail[$field['fieldname']]))? 
																	 ...
																	 :(($field['fieldname']=='leadaction' ) ? (($leadaccess == 1) ?
																	 (form_input(array(
																	  'class'	  => 'form-control'
																	  ,'name'      => $field['fieldname']
																	  ,'id'        => $field['fieldname']
																	...
																	.(form_input(array(
                                                                     ...
                                                                     :((in_array($field['fieldname'],array('record','replytocustomer','replytoexecutive','mailalerttowoner')))? 
																	form_checkbox(array('class'=>"switch",'data-size'=>"mini",
																                         'id'=>$field['fieldname']
																                         ,'name'=>$field['fieldname']
	...
	function manage_group($bid=''){
		...
		foreach($fieldset as $field){
			...
			"replytocust_missed","replytocust_attended","replytocust_repcal",'filename','bday','url','connectowner','hdayaudio','greetings','sameexe','recordnotice','misscall','pincode','allgroup','access_reports','voicemessage','replytocust_voitext','mailalerttowoner'
								))){
				foreach($roleDetail['system'] as $ret){if($ret['fieldid']==$field['fieldid'])$checked = true;}
				if($checked){ $formFields[] = array(
	...
	function blocklist(){
		...
		$data['links'] = $links; 
		$data['form'] = array(
							'open'=>form_open_multipart('BlockList/0',array('name'=>'blocklist','class'=>'form','id'=>'blocklist','method'=>'post')),
