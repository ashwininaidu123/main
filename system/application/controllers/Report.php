...
class Report extends controller
{
	...
	function call(){
	   ...
	   if(!$this->feature_access())redirect('Employee/access_denied');
		...
		$limit = '30';
	    $callfrom = '';
		if($this->uri->segment(2) =='CallLog'){
			$callfrom = $this->uri->segment(3);
			$ofset = ($this->uri->segment(4)!=null)?$this->uri->segment(4):0;
		}
		$data['itemlist'] = $this->reportmodel->getReportlist($bid,$ofset,$limit,$url,$u3,$callfrom);
		$dlink = "";
   ...
   function archive_records($id=''){
	...
	function followup($id='',$dsh = '',$type=''){
		...
		if($dsh == 1){
			...
			$this->load->view('popupFormView1',$data);
	    }
	}
	function missed(){
	...
	function callsummary(){
		...
		$list=$this->reportmodel->getCallSummary($search,$ofset,$limit,$source="web");
		$header = array(
		            '#'
					,'Group Name'
					,'Keyword'
  ...
  function CallbackUser(){ 
   ...
   function new_release(){
	  ...
	  $this->sysconfmodel->data['html']['title'] .= " | New Release List";
      $this->sysconfmodel->viewLayout('newrelease',$data);
	}
}
