...
Class Reportmodel extends Model
{
	...
	function getReportlist($bid,$ofset='0',$limit='20',$type='a',$tab='',$callfrom='')
	{
		...
		if($uniqueBy == '' && $callfrom == ''){
		    ...
		    $sql =  "SELECT SQL_CALC_FOUND_ROWS h2.callid,h2.hid,a.callfrom,
					...
					LEFT JOIN ".$bid."_employee k ON k.eid = a.assignto  
					".$ge." 
					WHERE a.status!=2  AND a.callfrom = $callfrom  ".$q.") as a
		...
		foreach($rst as $rec){
			...
			/*	if($_SERVER['REMOTE_ADDR']=='220.227.66.89'){
					echo "Filename:".$r['filename']."<br>";
					echo "Filen exists:".file_exists('sounds/'.$r['filename'])."<br>";
				}*/
				...
				//~ }elseif($k=='callfrom'){
					//~ $v = anchor("Report/clicktoconnect/".$r['callid']."/1","$r[$k]",array('class'=>'clickToConnect','title' =>'Click to connect'));
					 ...
					 if(strlen($r['callfrom'])>='12'){
						//$v = anchor("TrackReport/CallLog/".$r['callfrom']."","$r[$k]",array('class'=>'calllog','title' =>'Call Log'));
						$v = $r['callfrom'];
					}else{
						//$v = anchor("TrackReport/CallLog/".$r['callfrom']."",'0'."$r[$k]",array('class'=>'calllog','title' =>'Call Log'));
						$v ='0'.$r['callfrom'];
					}
				}elseif($k=='eid'){
 ...
 /* mail functionality for download */
	...
	function recent_calls($type,$gettimelimit){
		...
		$sql = $this->db->query("SELECT count(h.callid) as count,h.gid,g.groupname as gname,gr.groupname
									FROM ".$this->session->userdata('bid')."_callhistory h 
									LEFT JOIN ".$this->session->userdata('bid')."_groups g on h.gid=g.gid
									LEFT JOIN ".$this->session->userdata('bid')."_groups gr on h.gid=gr.gid
	...
	function call_types($type,$c='0',$gettimelimit){
		...
		$q=($type=='a')?"":"";
		$q1 =($type=='du')?"SUM(a.pulse) as cnt":"COUNT(a.callid) as cnt";
		$roleDetail = $this->empmodel->getRoledetail($this->session->userdata('roleid'));
		if($roleDetail['role']['admin']!=1){
...
//~ 
  ...
  function clicktoconnectedit($id,$bid){
	...
	function getDetail($bid,$dataid,$module,$eid){
		...
		//~ }
		if($ret == '1'){
			$st = (array)json_decode(file_get_contents("http://180.151.5.53/filter.php?num=".substr($callDetail['callfrom'],-10,10)));
		...
		if($ret == '1'){
			...
			$sql = "INSERT INTO click2connect SET
					...
					,eid		='".$empDetail['eid']."'";
			$this->db->query($sql);
			file("http://10.40.170.41/clicktocall.php?callid=".$callid);
		}
		return $ret;
	}
	...
	function sms_bal(){
		$sql=$this->db->query("SELECT balance FROM sms_bal  where bid='".$this->session->userdata('bid')."'");
		if($sql->num_rows()>0){
			$res = $sql->row();
			return $res->balance;
		}else{
	...
	function call_bal(){
		$sql=$this->db->query("SELECT balance FROM call_bal where bid='".$this->session->userdata('bid')."'");
		if($sql->num_rows()>0){
			$res = $sql->row();
			return $res->balance;
		}else{
...
function followupdownload($bid){
	...
	function offlineusers(){
		...
		$sql=$this->db->query("SELECT DISTINCT(e.eid),e.empname,eb.start_time FROM ".$this->session->userdata('bid')."_employee e
		                       LEFT JOIN ".$this->session->userdata('bid')."_emp_break eb ON e.eid = eb.eid
		                       LEFT JOIN ".$this->session->userdata('bid')."_groups d ON e.eid = d.eid
	...
	function ivrs_groupwise_todaycall($gettimelimit){
		...
		$sql=$this->db->query("SELECT count( i.ivrsid ) as count ,iv.title as title,iv1.title as groupname FROM ".$this->session->userdata('bid')."_ivrshistory i
							 LEFT JOIN ".$this->session->userdata('bid')."_ivrs iv ON i.ivrsid = iv.ivrsid
							 LEFT JOIN ".$this->session->userdata('bid')."_ivrs iv1 ON i.ivrsid = iv1.ivrsid
	...
	function lead_empgroupwise_todaycalls($gettimelimit){
		...
		$sql=$this->db->query("SELECT count( l.assignto ) AS count, e.empname AS employee,e1.empname, e.empnumber AS number, e.eid AS eid
								FROM `".$this->session->userdata('bid')."_leads` l
								LEFT JOIN `".$this->session->userdata('bid')."_employee` e ON l.assignto = e.eid
								LEFT JOIN `".$this->session->userdata('bid')."_employee` e1 ON l.assignto = e1.eid
	...
	function leads_assigned_detail($gettimelimit){
		...
		$sql=$this->db->query("SELECT l.name, l.email, l.number,date(l.createdon) as createdon,e.empname, e.empemail, e.empnumber
		                       FROM `".$this->session->userdata('bid')."_leads` l
		                       LEFT JOIN ".$this->session->userdata('bid')."_employee e ON l.assignto = e.eid
	...
	function support_empgroupwise_todaycalls($gettimelimit){
		...
		$sql=$this->db->query("SELECT count( st.assignto ) AS count, e.empname AS employee,e1.empname, e.empnumber AS number, e.eid AS eid
								FROM `".$this->session->userdata('bid')."_support_tickets` st
								LEFT JOIN `".$this->session->userdata('bid')."_employee` e ON st.assignto = e.eid
								LEFT JOIN `".$this->session->userdata('bid')."_employee` e1 ON st.assignto = e1.eid
	...
	function support_types($type,$gettimelimit){
		...
		$roleDetail = $this->empmodel->getRoledetail($this->session->userdata('roleid'));
		$q=($type=='o')?"AND st.status=1":"";
		$q.=($type=='p')?"AND st.status=2":"";
		$q.=($type=='r')?"AND st.status=3":"";
		$q.=($type=='c')?"AND st.status=4":"";
		if($roleDetail['role']['admin']!=1){
			$q .= " AND e.eid=".$this->session->userdata('eid')." ";
		}
		...
		$sql=$this->db->query("SELECT count( st.status ) AS cnt, ss.status FROM `".$this->session->userdata('bid')."_support_tickets` st
								...
								LEFT JOIN `".$this->session->userdata('bid')."_employee` e ON st.assignto = e.eid WHERE 1 ".$q);
		return ($sql->num_rows()>0) ?$sql->row()->cnt:0;
	}
	...
	function getCallSummary($search,$ofset,$limit,$source){
		$bid = $this->session->userdata('bid');
		$con = '';
		$search = array(
		...
		//$con = (isset($search['sdate']) && $search['gid']!='0') ? " WHERE g.gid='".$search['gid']."'" : "";
		$ret = array();
		$data = array();
  ...
  function empgroupwise_mtrack($gettimelimit){
   ...
   function employee_list(){
	...
	function apiRes($id,$bid){
		...
		foreach($rst as $rec){
			$data = array($i
						  ,"<textarea cols='45' rows='15'>".htmlentities(unserialize($rec['oncallResponse']))."</textarea>" //$rec['oncallResponse']
						  ,"<textarea cols='45' rows='15'>".htmlentities(unserialize($rec['onhangupResponse']))."</textarea>" //$rec['onhangupResponse']
						  );
			$i++;
			array_push($list,$data);
		}
	...
	function getCallLog($bid,$callfrom,$ofset='0',$limit='20'){
