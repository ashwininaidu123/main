...
Class Groupmodel extends Model
{
	...
	function getgrouplist($bid,$ofset='0',$limit='20'){
		...
		foreach($fieldset as $field){
			...
			if($field['type']=='s' && !$field['is_hidden'] && $field['show'] && $field['listing']){
				...
				if($checked){
				if(!in_array($field['fieldname'],array('hdayaudio','greetings','url','voicemessage','replytocust_missed','replytocust_attended','replytocust_repcal','mailalerttowoner'))){
			...
			if($field['type']=='s' && !$field['is_hidden'] && $field['show'] && $field['listing']){
				...
				if($checked){
					if(!in_array($field['fieldname'],array('hdayaudio','greetings','url','voicemessage','replytocust_missed',
											...
											'replytocust_repcal','mailalerttowoner'))){
						array_push($keys,$field['fieldname']);
						array_push($header,(($field['customlabel']!="")
											? $field['customlabel']
											: $this->lang->line('mod_'.$field['modid'])->$field['fieldname']));
	...
	function add_blacklistnumber(){
	    $number = $this->db->query("SELECT * FROM ".$this->session->userdata('bid')."_blocknumbers WHERE number = ".$this->input->post('blacklist')."");
